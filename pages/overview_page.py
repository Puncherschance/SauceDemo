...
class OverviewPage(BasePage):
    ...
    @allure.step(
        "Проверить, что отображается количество продукта {value} для каждого продукта."
    )
    ...
    def check_finish_button_has_text_(self, text: str):
    ...
    def click_product_name(self):
        self.click_element(locator=PRODUCT_1_NAME)
    ...
    def check_payment_information_header_has_text_(self, text: str):
    ...
    def check_card_number_has_text_(self, card_number: str):
        self.check_element_has_text(locator=CARD_NUMBER, data=card_number)

    @allure.step("Проверить, что присутствует текст заголовка: {text}.")
    def check_shipping_information_header_has_text_(self, text: str):
        self.check_element_has_text(locator=SHIPPING_INFO, data=text)

    @allure.step("Проверить, что присутствует цена доставки: {price}.")
    def check_shipping_price_has_text_(self, price: str):
        self.check_element_has_text(locator=SHIPPING_PRICE, data=price)

    @allure.step("Проверить, что присутствует цена доставки: {text}.")
    def check_price_total_header_has_text_(self, text: str):
        self.check_element_has_text(locator=PRICE_TOTAL, data=text)

    @allure.step(
        "Проверить, что присутствует корректный подитог стоимости заказа для продукта."
    )
    def check_item_total_correct(self, product_data: dict[str]):
        price = product_data["Price"][0]
        item_total = "Item total: " + price
        self.check_element_has_text(locator=SUBTOTAL, data=item_total)

    @allure.step(
        "Проверить, что присутствует корректный подитог стоимости заказа для продуктов."
    )
    def check_item_total_correct_multiple_products(self, products_data: tuple):
        sum_prices = self.calculate_item_total_for_6_products(products_data)
        item_total = "Item total: " + sum_prices
        self.check_element_has_text(locator=SUBTOTAL, data=item_total)

    @allure.step("Проверить, что присутствует корректный расчет налога для продукта.")
    def check_tax_correct(self, product_data: dict[str]):
        price = product_data["Price"][0]
        tax = "Tax: $" + self.calculate_tax(data=price)
        self.check_element_has_text(locator=TAX, data=tax)

    @allure.step("Проверить, что присутствует корректный расчет налога для продуктов.")
    def check_tax_correct_multiple_products(self, products_data: tuple):
        sum_prices = self.calculate_item_total_for_6_products(products_data)
        tax = "Tax: $" + self.calculate_tax(data=sum_prices)
        self.check_element_has_text(locator=TAX, data=tax)

    @allure.step("Проверить, что присутствует корректный расчет итога для продукта.")
    def check_total_correct(self, product_data: dict[str]):
        price = product_data["Price"][0]
        tax = self.calculate_tax(data=price)
        total = "Total: $" + self.calculate_item_total_for_product(price=price, tax=tax)
        self.check_element_has_text(locator=TOTAL, data=total)

    @allure.step("Проверить, что присутствует корректный расчет итога для продуктов.")
    def check_total_correct_multiple_products(self, products_data: tuple):
        sum_prices = self.calculate_item_total_for_6_products(products_data)
        tax = self.calculate_tax(data=sum_prices)
        total = "Total: $" + self.calculate_item_total_for_product(
            price=sum_prices, tax=tax
        )
        self.check_element_has_text(locator=TOTAL, data=total)
