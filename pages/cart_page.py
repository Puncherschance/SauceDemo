...
class CartPage(BasePage):
    ...
    def check_product_data_not_shown(self):
        for locator in (PRODUCT_NAME, PRODUCT_DESC, PRODUCT_PRICE, PRODUCT_QTY, REMOVE):
            self.check_element_not_shown(locator=locator)

    @allure.step(
        'Проверить, что отображается количество кнопок "Remove" равное {value}.'
    )
