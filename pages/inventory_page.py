...
class InventoryPage(BasePage):
    ...
    @allure.step("Добавить первый по списку продукт в корзину.")
    def add_specific_product_to_cart(self):
        self.click_element(locator=PRODUCT_1_CART)

    @allure.step("Добавить продукт в корзину.")
    def add_product_to_cart(self, product: dict[str, str]):
    ...
    @allure.step(
        "Проверить, что присутствуют все доступные опции сортировки продуктов."
    )
    def check_all_sorting_options_shown(self):
        self.check_element_has_text(
            locator=SORT_MENU,
            data="Name (A to Z)Name (Z to A)Price (low to high)Price (high to low)",
        )
    ...
    def expand_product_description_via_clicking_product_name(
        self, product_data: dict[str, str]
    ):
        locator = product_data["Name"][1]
        self.click_element(locator=locator)

    @allure.step(
        "Открыть подробное описание продукта, кликнув на изображение продукта."
    )
    def expand_product_description_via_clicking_product_image(
        self, product_data: dict[str, str]
    ):
        locator = product_data["Image"]
        self.click_element(locator=locator)
    ...
    @allure.step(
        'Проверить, что текст всех кнопок "Add to сart" изменился на "Remove".'
    )
    def check_remove_buttons_shown(self):
        for item in INVENTORY_ITEMS:
            locator = item["Add to cart Button"]
            self.check_element_has_text(locator=locator, data="Remove")

    @allure.step(
        'Проверить, что текст всех кнопок "Remove" изменился на "Add to сart".'
    )
