...
class TestHeader:
    ...
    def test_swag_labs_tittle_shown(self, page: Page, overview_page):
...
class TestHamburgerMenu:
    ...
    def test_check_hamburger_option_has_correct_text(self, page: Page, overview_page, option):
    ...
    def test_logout_option_leads_to_login_page_and_logout_user(self, page: Page, overview_page):
        overview_page.open_hamburger_menu()
...
class TestProducts:
    ...
    def test_product_descriptions_shown(self, page: Page, two_random_products, overview_page):
    ...
    @allure.title('Проверить, что кнопка "Cancel" редиректит пользователя на страницу Inventory Page.')
    def test_cancel_button_leads_to_inventory_page(self, page: Page, overview_page):
        overview_page.click_cancel_button()
        overview_page.check_inventory_page_opened()

    @allure.title('Проверить, что кнопка "Finish" редиректит пользователя на страницу Complete Page.')
...
class TestPaymentData:
    ...
    @allure.title('Проверить, что присутствует колонка "Payment Information" с корректной информацией.')
    ...
    @allure.title('Проверить, что присутствует колонка "Shipping Information" с корректной информацией.')
    def test_shipping_information_shown(self, page: Page, overview_page):
        overview_page.check_shipping_information_header_has_text_('Shipping Information:')
        overview_page.check_shipping_price_has_text_('Free Pony Express Delivery!')

    @allure.title('Проверить, что присутствует колонка "Price Total" с корректной информацией.')
...
class TestFooter:
    ...
    def test_terms_of_service_has_text(self, page: Page, overview_page):
        overview_page.check_terms_of_service_has_text_('© 2024 Sauce Labs. All Rights Reserved. Terms of Service | Privacy Policy')
