...
class TestCart:
    ...
    @allure.title('Проверить, что бейджик на иконке "Тележка" не отображается, если у пользователя пустая корзина.')
    def test_cart_icon_shown_without_badge_when_empty_cart(self, page: Page, cart_page):
        cart_page.check_cart_badge_value_not_shown()

    @allure.title('Проверить, что кнопка "Continue Shopping" редиректит пользователя на страницу Inventory page.')
    ...
    @allure.title('Проверить, что пользователь остается на странице Cart page, если кликнуть на иконку "Тележка".')
    ...
    @allure.title('Проверить, что пользователь не переходит к покупке с пустой корзиной.')
