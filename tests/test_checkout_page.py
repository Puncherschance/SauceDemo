...
class TestHeader:
    ...
    def test_swag_labs_title_shown(self, page: Page, checkout_page):
    ...
    def test_page_title_shown(self, page: Page, checkout_page):
...
class TestHamburgerMenu:
    ...
    def test_check_hamburger_option_has_correct_text(
        self, page: Page, checkout_page, option
    ):
    ...
    def test_close_hamburger_menu(self, page: Page, checkout_page):
        checkout_page.open_hamburger_menu()
        checkout_page.check_close_hamburger_menu_button_shown()
        checkout_page.close_hamburger_menu()
        checkout_page.check_hamburger_menu_closed()
...
class TestRequisites:
    ...
    @allure.title(
        'Проверить, что кнопка "Cancel" редиректит пользователя на страницу Cart Page.'
    )
    ...
    @allure.title(
        'Проверить, что кнопка "Continue" редиректит пользователя на страницу Overview Page, если заполнены все '
        'реквизиты.'
    )
...
class TestValidation:
    ...
    @allure.title(
        "Проверить, что появляется валидация, если попытаться перейти к покупке, оставив все поля пустыми на форме "
        "ввода реквизитов."
    )
    def test_validation_shown_if_proceed_checkout_with_empty_data(
        self, page: Page, checkout_page
    ):
        checkout_page.click_continue_button()
    ...
    @allure.title(
        'Проверить, что появляется валидация, если попытаться перейти к покупке, оставив поле "First Name" пустым на '
        'форме ввода реквизитов.'
    )
    def test_validation_shown_if_proceed_checkout_with_empty_first_name(
        self, page: Page, checkout_page
    ):
        checkout_page.enter_last_name_("User")
        checkout_page.enter_postal_code_("41000")
    ...
    @allure.title(
        'Проверить, что появляется валидация, если попытаться перейти к покупке, оставив поле "Last Name" пустым на '
        'форме ввода реквизитов.'
    )
    def test_validation_shown_if_proceed_checkout_with_empty_last_name(
        self, page: Page, checkout_page
    ):
    ...
    @allure.title(
        'Проверить, что появляется валидация, если попытаться перейти к покупке, оставив поле "Zip/Postal Code" '
        'пустым на форме ввода реквизитов.'
    )
    def test_validation_shown_if_proceed_checkout_with_empty_postal_code(
        self, page: Page, checkout_page
    ):
        checkout_page.enter_first_name_("User")
        checkout_page.enter_last_name_("User")
...
class TestFooter:
    ...
    def test_terms_of_service_has_text(self, page: Page, checkout_page):
        checkout_page.check_terms_of_service_has_text_(
