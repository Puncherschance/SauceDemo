...
class TestAuthForm:
    ...
    @allure.title('Проверить, что удается успешно авторизоваться в систему стандартным пользователем.')
...
class TestValidation:
    ...
    @allure.title('Проверить, что отображается валидация, если не вводить логин и пароль.')
    ...
    @allure.title('Проверить, что отображается валидация, если попытаться авторизоваться в систему заблокированным пользователем.')
    ...
    @allure.title('Проверить, что отображается валидация, если попытаться авторизоваться в систему с некорректными реквизитами.')
    ...
    @allure.title('Проверить, что остальные страницы доступны только после авторизации.')
    @pytest.mark.parametrize('page_endpoint', PAGES)
    def test_page_reached_only_after_authorization(self, page, login_page, page_endpoint):
        login_page.open_page_by_direct_url(page_endpoint)
        login_page.check_validation_has_text_(f"Epic sadface: You can only access '/{page_endpoint}' when you are logged in.")
